...
echo Rules of Firewall 
echo.
PowerShell -NoProfile -ExecutionPolicy Bypass -Command "& {Start-Process PowerShell -ArgumentList '-NoProfile -ExecutionPolicy Bypass -File ""%~dp0.\blocklist.ps1""' -Verb RunAs}"
echo Rules included in Firewall...
echo.
echo DELETE DiagTrack and dmwappushservice?
pause > NUL
sc delete DiagTrack
...
echo Also experimental?
pause > NUL
EM *** Scans startup entries and raises notification to the user if there are too many startup entries ***
